...
#rom spellchecker import SpellChecker
...
# f = open(out_prefix + '.unbiased', 'w')
s = []
with open(crawl_path) as file:
  ...
  for l in lines:
    ...
    tok_s = [tokenize(s_i) for s_i in s]
    s2 = sent_tokenize(line[2])
    tok_s2 = [tokenize(s_i) for s_i in s2]
    with open(out_file, "a") as outfile:
      tok_s_str = ' '.join(tok_s)
      tok_s2_str = ' '.join(tok_s2)
      # print('tok', tok_s_str)
      outfile.writelines(line[0] + "\t" + tok_s_str + "\t" + tok_s2_str + "\t" + "a" + "\t" + "b" + "\n")
